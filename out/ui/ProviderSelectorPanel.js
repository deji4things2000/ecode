...
exports.ProviderSelectorPanel = void 0;
...
// ─────────────────────────────────────────────
class ProviderSelectorPanel {
    constructor(registry, context) {
        this.registry = registry;
        this.context = context;
        this.isDisposed = false;
        this.panel = vscode.window.createWebviewPanel('aiAgentProviders', '🤖 AI Agent — Select Provider', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    // ── Singleton factory ─────────────────────────
    static create(registry, context) {
        ...
        this.panel.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                ...
                case 'installProvider':
                    await this.handleInstall(msg.id);
                    break;
                case 'startProvider':
                    await this.handleStart(msg.id);
                    break;
                case 'enterApiKey':
                    await this.handleApiKey(msg.id);
                    break;
                case 'openUrl':
                ...
                case 'refreshStatuses':
                    ...
                    break;
                case 'selectModel':
                    await this.handleModelSelect(msg.id, msg.model);
                    break;
            }
        });
    }
    ...
    async handleSelect(id) {
        ...
        if (meta.status === 'not-installed') {
            const answer = await vscode.window.showWarningMessage(`${meta.icon} ${meta.displayName} is not installed.`, 'Install Now', 'Open Download Page', 'Cancel');
            if (answer === 'Install Now') {
                await this.handleInstall(id);
            }
        ...
        if (meta.status === 'not-running') {
            const answer = await vscode.window.showWarningMessage(`${meta.icon} ${meta.displayName} is installed but not running.`, 'Start Server', 'Cancel');
            if (answer === 'Start Server') {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`${meta.icon} Switched to ${meta.displayName}`);
            // Refresh to show updated active state
            this.sendProviderData();
        }
        catch (err) {
            vscode.window.showErrorMessage(`Failed to switch to ${meta.displayName}: ${err.message}`);
        }
    }
    async handleInstall(id) {
        const meta = this.registry.getMeta(id);
        this.post({ command: 'setStatus', id, status: 'installing' });
        const started = await this.registry.autoInstall(id);
        if (started) {
            vscode.window.showInformationMessage(`${meta.icon} Installing ${meta.displayName} in terminal…\n` +
                `Switch back to the provider panel after installation completes.`);
            // Poll status every 5 s for up to 2 minutes
            this.pollStatus(id, 5000, 24);
        }
        else {
    ...
    async handleStart(id) {
        ...
        this.pollStatus(id, 2000, 15);
        vscode.window.showInformationMessage(`${meta.icon} Starting ${meta.displayName}…`);
    }
    async handleApiKey(id) {
        const meta = this.registry.getMeta(id);
        ...
        const key = await vscode.window.showInputBox({
            ...
            validateInput: v => !v || v.length < 10 ? 'Key is too short' : null,
        });
        if (!key) {
            return;
        }
        ...
        vscode.window.showInformationMessage(`${meta.icon} API key saved for ${meta.displayName}`);
    ...
    async handleModelSelect(id, model) {
        const config = vscode.workspace.getConfiguration('aiAgent');
    ...
    // ── Polling ───────────────────────────────────
    pollStatus(id, intervalMs, maxAttempts) {
        let attempts = 0;
        const timer = setInterval(async () => {
            ...
            if (status === 'ready' || attempts >= maxAttempts) {
                ...
                this.sendProviderData();
                if (status === 'ready') {
                    vscode.window.showInformationMessage(`${meta.icon} ${meta.displayName} is ready!`);
                }
            }
        }, intervalMs);
    }
...
exports.ProviderSelectorPanel = ProviderSelectorPanel;
//# sourceMappingURL=ProviderSelectorPanel.js.map